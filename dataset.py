...
class MelanomaDataset(Dataset):
    def __init__(self, csv, mode, meta_features, transform=None, image_path=None):

        self.csv = csv.reset_index(drop=True) if csv is not None else csv
        self.mode = mode
    ...
    def __len__(self):
        return self.csv.shape[0] if self.csv is not None else 1
