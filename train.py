...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, mel_idx):
    ...
    if args.DEBUG:
        args.n_epochs = 5
        df_train = pd.concat([
            df[(df['fold'] != fold) & (df['target'] != mel_idx)].sample(args.batch_size * 5),
            df[(df['fold'] != fold) & (df['target'] == mel_idx)].sample(args.batch_size * 5)
        ])
        df_valid = pd.concat([
            df[(df['fold'] == fold) & (df['target'] != mel_idx)].sample(args.batch_size * 5),
    ...
    print(df_train.target.value_counts(), df_valid.target.value_counts())
...
#         scheduler_warmup.step(epoch - 1)
        ...
        content = time.ctime() + ' ' + f'Fold {fold}, Epoch {epoch}, lr: {optimizer.param_groups[0]["lr"]:.7f}, ' \
                  f'train loss: {train_loss:.5f}, valid loss: {(val_loss):.5f}, acc: {(acc):.4f}, ' \
                  f'auc: {(auc):.6f}, auc_20: {(auc_20):.6f}.'
        print(content)
